...
fn main() {
    ...
    //creates a file of 498 unsorted ints
    //feel free to comment this block out if you have a custom list of ints needing to be sorted
    let mut input_file: fs::File = fs::File::create("unsorted.txt").expect("Could not create file");
    for _i in 1..500 {
        //generates an int and writes it to a new line in the unsorted.txt file
        write!(input_file, "{}\n", rng.gen_range(1..=500)).expect("Could not write to file");
    }
    println!("List Generated");


    //reads a file of unsorted ints
    ...
    let mut output_file: fs::File = fs::File::create("output.txt").expect("Could not create file");
    
    //writes the sorted ints to the file each on an individual line
